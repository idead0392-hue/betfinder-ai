...
def get_esport_team_image(team_id, api_key, api_host):
    """
    Get esport team image using RapidAPI.
    ...
    conn.request("GET", f"/api/esport/team/{team_id}/image", headers=headers)
    res = conn.getresponse()
    data = res.read()
    return data.decode("utf-8")

def get_esport_team_previous_matches(team_id, page, api_key, api_host):
    """
    Get esport team previous matches using RapidAPI.
    ...
    conn.request("GET", f"/api/esport/team/{team_id}/matches/previous/{page}", headers=headers)
    res = conn.getresponse()
    data = res.read()
    return data.decode("utf-8")

def get_esport_team_near_matches(team_id, api_key, api_host):
    """
    Get esport team near matches using RapidAPI.
    ...
    conn.request("GET", f"/api/esport/team/{team_id}/matches/near", headers=headers)
    res = conn.getresponse()
    data = res.read()
    return data.decode("utf-8")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("Esport Team Image API Test")
    print("=" * 60 + "\n")
    result = get_esport_team_image('372647', RAPIDAPI_KEY, RAPIDAPI_HOST)
    print(result)
    
    print("\n" + "=" * 60)
    print("Esport Team Previous Matches API Test")
    print("=" * 60 + "\n")
    result = get_esport_team_previous_matches('459469', '1', RAPIDAPI_KEY, RAPIDAPI_HOST)
    print(result)
