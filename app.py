...
# Import sport agents for esports picks generation
from sport_agents import create_sport_agent
...
def generate_esports_props(game_type, max_props=10):
    """Generate esports props using sport agents"""
    try:
        # Create agent for the specific esports game
        agent = create_sport_agent(game_type)
        
        # Generate picks
        picks = agent.fetch_props(max_props)
        
        # Limit the number of props
        return picks[:max_props]
    except Exception as e:
        st.error(f"Error generating {game_type} props: {e}")
        return []

def display_esports_props(game_type, game_name):
    """Display esports props for a specific game"""
    st.markdown(f'<div class="section-title">{game_name}<span class="time">AI-Generated Props</span></div>', unsafe_allow_html=True)
    
    # Generate props using the sport agent
    props = generate_esports_props(game_type, max_props=15)
    
    if props:
        st.success(f"🎮 Generated {len(props)} {game_name} props using AI analysis")
        
        # Display props in a nice format
        for i, prop in enumerate(props):
            # Calculate confidence based on form and difficulty (placeholder algorithm)
            confidence = min(95, max(55, (prop['recent_form'] * 5) + (10 - prop['matchup_difficulty']) * 4))
            
            # Determine over/under based on odds
            over_under = "Over" if prop['odds'] > 0 else "Under"
            pick_text = f"{prop['player_name']} {over_under} {prop['line']} {prop['stat_type']}"
            
            with st.expander(f"🎯 {pick_text} ({confidence:.1f}% confidence)", expanded=i < 3):
                col1, col2, col3 = st.columns([2, 1, 1])
                
                with col1:
                    st.write(f"**Player:** {prop['player_name']}")
                    st.write(f"**Matchup:** {prop['matchup']}")
                    st.write(f"**Event Time:** {prop.get('event_start_time', 'TBD')}")
                    st.write(f"**Map:** {prop.get('map', 'TBD')}")
                    if 'side_preference' in prop:
                        st.write(f"**Side Preference:** {prop['side_preference']}")
                
                with col2:
                    st.metric("Confidence", f"{confidence:.1f}%")
                    st.metric("Recent Form", f"{prop['recent_form']:.1f}/10")
                    st.write(f"**Odds:** {prop['odds']:+d}" if prop['odds'] != 0 else "**Odds:** EVEN")
                
                with col3:
                    st.write(f"**Stat:** {prop['stat_type'].upper()}")
                    st.write(f"**Line:** {prop['line']}")
                    st.write(f"**Pick:** {over_under}")
                    st.write(f"**Sportsbook:** {prop['sportsbook']}")
                ...
                with st.container():
                    ...
                    with analysis_col1:
                        st.caption(f"• Recent Form: {prop['recent_form']:.1f}/10")
                        st.caption(f"• Matchup Difficulty: {prop['matchup_difficulty']:.1f}/10")
                        st.caption(f"• Injury Status: {prop.get('injury_status', 'Unknown').title()}")
                    
                    with analysis_col2:
                        if 'team_chemistry' in prop:
                            st.caption(f"• Team Chemistry: {prop['team_chemistry']:.2f}")
                        if 'recent_map_performance' in prop:
                            st.caption(f"• Map Performance: {prop['recent_map_performance']:.2f}")
                        if 'opponent_map_ban_rate' in prop:
                            st.caption(f"• Opponent Map Ban Rate: {prop['opponent_map_ban_rate']:.2f}")
        ...
        try:
            ...
            if summary['performance_metrics']:
                ...
                with col1:
                    st.metric("Total Picks", metrics.get('total_picks', 0))
                with col2:
        ...
        st.warning(f"⚠️ No {game_name} props available")
        st.info(f"Unable to generate {game_name} props at this time. The AI agent may need more data.")
    
    # Add timestamp
    from datetime import datetime
    current_time = datetime.now().strftime("%H:%M:%S")
    st.caption(f"{game_name} props generated by AI agent at {current_time} • Updates with new analysis")
...
tab_names = [
    ...
    "Hockey", "Soccer", "CS:GO", "League of Legends", "Dota 2", "VALORANT", "Overwatch", "Rocket League", "College Football"
]
...
# CS:GO TAB
with tabs[9]:
    display_esports_props('csgo', 'CS:GO')

# LEAGUE OF LEGENDS TAB  
with tabs[15]:
    display_esports_props('league_of_legends', 'League of Legends')

# DOTA 2 TAB
with tabs[11]:
    display_esports_props('dota2', 'Dota 2')

# VALORANT TAB
with tabs[12]:
    display_esports_props('valorant', 'VALORANT')

# OVERWATCH TAB
with tabs[13]:
    display_esports_props('overwatch', 'Overwatch')

# ROCKET LEAGUE TAB
with tabs[14]:
    display_esports_props('rocket_league', 'Rocket League')
